...
public class OboeTrack {
    static {
        System.loadLibrary("OboeTrack");
    }

    AudioTrack comparison = new AudioTrack(
            STREAM_MUSIC,
            44100,
            CHANNEL_OUT_STEREO,
            ENCODING_PCM_16BIT,
    ...
    private long native_OboeTrack_pointer = 0;
    private native long createNativeInstance();
    private native int getSampleRate(long native_OboeTrack_pointer);
    private native int getChannelCount(long native_OboeTrack_pointer);
    private native int getUnderrunCount(long native_OboeTrack_pointer);
    private native int getCurrentFrame(long native_OboeTrack_pointer);
    private native int getTotalFramesFrame(long native_OboeTrack_pointer);
    private native void setTrack(long native_OboeTrack_pointer, String track);
    private String converted;

    public int getSampleRate() {
        return getSampleRate(native_OboeTrack_pointer);
    }

    public int getChannelCount() {
        return getChannelCount(native_OboeTrack_pointer);
    }

    public int getUnderrunCount() {
        return getUnderrunCount(native_OboeTrack_pointer);
    }

    public int getCurrentFrame() {
        return getCurrentFrame(native_OboeTrack_pointer);
    }

    public int getTotalFrames() {
        return getTotalFramesFrame(native_OboeTrack_pointer);
    }

    public OboeTrack() {
        native_OboeTrack_pointer = createNativeInstance();
    }

    private void _load(Path tmp) {
        int sampleRate = getSampleRate(native_OboeTrack_pointer);
        int channelCount = getChannelCount(native_OboeTrack_pointer);
        ...
        if (returnCode == RETURN_CODE_SUCCESS) {
            ...
            setTrack(native_OboeTrack_pointer, converted);
        } else {
