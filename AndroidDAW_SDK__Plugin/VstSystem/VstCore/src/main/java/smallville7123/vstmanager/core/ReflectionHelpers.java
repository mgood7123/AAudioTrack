...
public class ReflectionHelpers {
     ...
     * @return The return value of the method.
     */
    public static <R> R callPublicInstanceMethod(final Object instance, final String methodName, ClassParameter<?>... classParameters) {
        try {
            final Class<?>[] classes = ClassParameter.getClasses(classParameters);
            final Object[] values = ClassParameter.getValues(classParameters);
            ...
            return traverseClassHierarchy(instance.getClass(), NoSuchMethodException.class, new InsideTraversal<R>() {
                ...
                @SuppressWarnings("unchecked")
                public R run(Class<?> traversalClass) throws Exception {
                    Method declaredMethod = traversalClass.getMethod(methodName, classes);
                    declaredMethod.setAccessible(true);
                    return (R) declaredMethod.invoke(instance, values);
                }
            });
    ...
    public static <R> R callInstanceMethod(Class<?> cl, final Object instance, final String methodName, ClassParameter<?>... classParameters) {
        final Object[] returnValue = new Object[]{null};
        traverseClass(cl, new TraversalRunnable() {
            @Override
            boolean run(Object argument) {
                try {
                    ...
                    returnValue[0] = method.invoke(instance, values);
                    return true;
                } catch (Exception e) {
                    return false;
                }
    ...
    public abstract static class TraversalRunnable {
        abstract boolean run(Object argument);
    }

    /**
     * traverses a given {@link Class} by {@link Class#getSuperclass() Superclass} until null is
     * returned or <b>r</b> returns true
     * @param aClass a {@link Class} object, obtained by <b>.class</b> or {@link Class#getClass()}
     * @param r the user should cast <b>argument</b> in <b>run(Object argument)</b>
     *          to type {@link Class}
     * @return the return value of <b>r</b>
     */
    public static boolean traverseClass(Class aClass, TraversalRunnable r) {
        Class target = aClass;
        while(target != null) {
            if (r.run(target)) return true;
            target = target.getSuperclass();
        }
        return false;
    }

    public static boolean containsInterface(Class aClass, String interfaceName) {
        return traverseClass(aClass, new TraversalRunnable() {
            @Override
            boolean run(Object argument) {
                for (Class anInterface : ((Class) argument).getInterfaces()) {
                    if (anInterface.getName().contentEquals(interfaceName)) {
                        return true;
                    }
                }
                return false;
    ...
    public static boolean classAextendsB(Class A, Class B) {
        return traverseClass(A, new TraversalRunnable() {
            @Override
            boolean run(Object argument) {
                return A == argument;
            }
