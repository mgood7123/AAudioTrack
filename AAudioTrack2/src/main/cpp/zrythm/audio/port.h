...
port_get_dest_index (
        ...
        Port * dest)
{
    if (!IS_PORT (self) || !IS_PORT (dest) ||
        !self->is_project || !dest->is_project) return -1;
...
port_get_src_index (
        ...
        Port * src)
{
    if (!IS_PORT (self) || !IS_PORT (src) ||
        !self->is_project || !src->is_project) return -1;
