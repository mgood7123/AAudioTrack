...
public:
    ...
    bool write(PortUtils2 * in, PortUtils2 * out, unsigned int samples) {
        ...
        for (int i = 0; i < out->ports.samples; i += 2) {
            reinterpret_cast<ENGINE_FORMAT *>(out->ports.outputStereo->l->buf)[i] = right.delayline(reinterpret_cast<ENGINE_FORMAT *>(in->ports.outputStereo->l->buf)[i]);
            reinterpret_cast<ENGINE_FORMAT *>(out->ports.outputStereo->r->buf)[i] = left.delayline(reinterpret_cast<ENGINE_FORMAT *>(in->ports.outputStereo->r->buf)[i]);
        }
        return true;
    }
};
