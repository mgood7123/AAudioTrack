...
import smallville7123.UI.TextViewResizeUtils.ResizeMode;
import smallville7123.aaudiotrack2.R;

/**
 * SLACASET - Single Line Auto Complete Auto Size EditText
 * an EditText that auto adjusts text size to fit on a single line within the view.
 */
@SuppressLint("AppCompatCustomView")
public class SLACASET extends PACET {

    private static final String TAG = "SLACASET";

    // Default constructor override
    public SLACASET(Context context) {
        super(context);
        init(context, null);
    }

    // Default constructor when inflating from XML file
    public SLACASET(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    // Default constructor override
    public SLACASET(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
    }

    public SLACASET(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            textViewResizeUtils.resizeMode = ResizeMode.width;
        }
